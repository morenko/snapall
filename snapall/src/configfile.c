...
ConfigFile *snap_cfg_open_file(gchar * filename)
{
	...
	while (lines[i])
	{
		if (lines[i][0] == '[')
		{
			...
			{
				*tmp = '\0';
				section = snap_cfg_create_section(cfg, &lines[i][1]);
			}
		}
		else if (lines[i][0] != '#' && section)
		{
			if ((tmp = strchr(lines[i], '=')))
			{
				*tmp = '\0';
				tmp++;
				snap_cfg_create_string(section, lines[i], tmp);
			}
		}
		i++;
	}
...
ConfigFile * snap_cfg_open_default_file(void)
{
	...
	ret = snap_cfg_open_file(snap_cfg_get_default_filename());
	if (!ret)
		ret = snap_cfg_new();
	return ret;
}
...
{
	return snap_cfg_write_file(cfg, snap_cfg_get_default_filename());
}
...
gboolean snap_cfg_read_string(ConfigFile * cfg, gchar * section, gchar * key, gchar ** value)
{
	...
	if (!(sect = snap_cfg_find_section(cfg, section)))
	return FALSE;
	if (!(line = snap_cfg_find_string(sect, key)))
		return FALSE;
...
gboolean snap_cfg_read_int(ConfigFile * cfg, gchar * section, gchar * key, gint * value)
{
	...
	if (!snap_cfg_read_string(cfg, section, key, &str))
		return FALSE;
	*value = atoi(str);
	g_free(str);
...
void snap_cfg_write_string(ConfigFile * cfg, gchar * section, gchar * key, gchar * value)
{
	...
	sect = snap_cfg_find_section(cfg, section);
	if (!sect)
		sect = snap_cfg_create_section(cfg, section);
	if ((line = snap_cfg_find_string(sect, key)))
	{
		g_free(line->value);
		...
		snap_cfg_create_string(sect, key, value);
}
...
void snap_cfg_write_int(ConfigFile * cfg, gchar * section, gchar * key, gint value)
{
	...
	snap_cfg_write_string(cfg, section, key, strvalue);
	g_free(strvalue);
}
...
void snap_cfg_write_boolean(ConfigFile * cfg, gchar * section, gchar * key, gboolean value)
{
	...
	if (value)
		snap_cfg_write_string(cfg, section, key, "TRUE");
	else
		snap_cfg_write_string(cfg, section, key, "FALSE");
}
...
void snap_cfg_remove_key(ConfigFile * cfg, gchar * section, gchar * key)
{
	...
	if ((sect = snap_cfg_find_section(cfg, section)) != NULL)
	{
		if ((line = snap_cfg_find_string(sect, key)) != NULL)
		{
			g_free(line->key);
			g_free(line->value);
			g_free(line);
